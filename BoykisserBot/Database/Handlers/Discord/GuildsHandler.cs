...
public class GuildsHandler(string connectionString) : BaseHandler(connectionString)
{
    ...
    public async Task<GuildsRow?> Get(ulong id)
    {
        ...
        return !reader.Read() ? null : new GuildsRow(ConnectionString, Config!, Handlers, reader);
    }
    ...
    public async Task<IReadOnlyList<GuildsRow>> GetAll()
    {
        ...
        while (await reader.ReadAsync()) guilds.Add(new GuildsRow(ConnectionString, Config!, Handlers, reader));

    ...
    : BaseRow(connectionString, handlersGroup, reader)
{
    ...
    public bool MessageTracking
    {
        get
        {
            using NpgsqlConnection? connection = GetConnection();
            using NpgsqlCommand? command = connection.CreateCommand();
            command.CommandText = "SELECT message_tracking FROM discord.users WHERE id = @id;";
            command.Parameters.Add(new NpgsqlParameter("id", NpgsqlDbType.Numeric) { Value = (long)Id });
        ...
        set
        {
            using NpgsqlConnection? connection = GetConnection();
            using NpgsqlCommand? command = connection.CreateCommand();
            command.CommandText = "UPDATE discord.users SET message_tracking = @value WHERE id = @id;";
            command.Parameters.Add(new NpgsqlParameter("id", NpgsqlDbType.Numeric) { Value = (long)Id });
